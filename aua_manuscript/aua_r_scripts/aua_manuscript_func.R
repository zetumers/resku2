...
#get the ratios
ratio <- function(long_df, fact_a, fact_b){
        long_df %>%
                group_by_(fact_a, fact_b) %>%
                summarize(count = n()) -> ans_df
        result <- ans_df[,names(ans_df) == fact_a]
        risk <- ans_df[,names(ans_df) == fact_b]
        count <- ans_df$count
        num <- count[result == 1 & risk == 1]/count[result == 0 & risk == 1]
        dem <- count[result == 1 & risk == 0]/count[result == 0 & risk == 0]
        ratio <- num/dem
        return(ratio)
}

#get the ratios
ratio2 <- function(long_df, fact_a, fact_b, reference, value){
        names(long_df)[names(long_df) == fact_a] <- "result"
        names(long_df)[names(long_df) == fact_b] <- "risk"
        long_df %>%
                group_by(result, risk) %>%
                summarize(count = n()) -> ans_df
        count <- ans_df$count
        risk <- ans_df$risk
        result <- ans_df$result
        num <- count[result == 1 & risk == value]/count[result == 0 & risk == value]
        dem <- count[result == 1 & risk == reference]/count[result == 0 & risk == reference]
        ratio <- num/dem
        return(ratio)
}
